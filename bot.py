...
def parse_expenses(text: str) -> list[dict]:
    ...
    Парсит строки вида:
      апельсины 500
      хлеб 45 руб, молоко 89
      эклеры - 13
      купили яблоки за 200 и сок 60
    """
    results = []
    # Split by comma, semicolon, newline or "и"
    parts = re.split(r'[,;\n]|(?<!\w)и(?!\w)', text)
    for part in parts:
        part = part.strip()
        if not part:
...
async def cmd_chart(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return
    if len(rows) < 2:
        await update.message.reply_text("📊 Нужно хотя бы 2 дня данных.")
        return
    buf = build_chart(rows)
    await update.message.reply_photo(photo=InputFile(buf, filename="chart.png"), caption="📊 Расходы за последние дни")
    ...
    if caption:
        ...
        if items:
            ...
            if saved:
                ...
                return
    await update.message.reply_text(
        "📸 Фото получено! Напишите расходы подписью к фото или отдельным сообщением:\n`апельсины 150`",
        parse_mode="Markdown"
    )
